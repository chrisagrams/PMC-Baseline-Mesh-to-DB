...
def parse_pubmed_xml(file_path: str):
    ...
    with gzip.open(file_path, 'rt', encoding='utf-8') as f:
        ...
        for article in tqdm(root.findall(".//PubmedArticle"), desc=f"Processing {os.path.basename(file_path)}"):
    ...
    for article in articles:
        try:
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (article["pmcid"], article["pmid"], article["doi"], article["title"], article["journal"]))
            ...
            for mesh in article["mesh_terms"]:
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    mesh["pmcid"], mesh["descriptor"], mesh["ui"], mesh["major"],
                    mesh["qualifier"], mesh["qual_ui"], mesh["qual_major"]
                ))
...
def process_directory(directory: str):
    ...
    files = [os.path.join(directory, f) for f in os.listdir(directory) if f.endswith(".xml.gz")]
